...
function cartProducts(){
    ...
    if(!cart){
        ...
        <td class="product-quantity"><input onchange=quantity('${JSON.stringify(cart.products[i])}') id="${cart.products[i].id}" min="1" type="number" value="${cart.products[i].quantityOnline}" /></td>
        <td class="product-subtotal">Rs ${cart.products[i].price*cart.products[i].quantityOnline}</td>
...
function quantity(product) {
    ...
    if (x > parsedProduct.quantityOnline) {
            let cart = getCartFromLocalStorage();
            ...
            cartProducts();
    } else if(x < parsedProduct.quantityOnline) {
        let cart = getCartFromLocalStorage();
            let ind = cart.products.findIndex(el=> el.id == parsedProduct.id)
            cart.products[ind].quantityOnline--;
            cart.products[ind].stock++;
